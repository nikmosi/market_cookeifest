...
def multithreadExec(funcs: list):
    ...
    for i in range(len(funcs)):
        threads.append(threading.Thread(target = lambda: setValueInList(results, i, funcs[i][0](*funcs[i][1]))))
        threads[-1].start()
...
def getProductDataByArticle(article: str):
    ...
    for step in steps:
        results.extend(multithreadExec([[requests.get, [f"https://basket-{('0' + str(i)) if (i < 10) else str(i)}.wbbasket.ru/vol{str(int(full_article[ : 4]))}/part{str(int(full_article[ : 6]))}/{article}/info/ru/card.json"]] for i in range(1 + len(results), 1 + len(results) + step)]))
...
def findTitleImageUrlByArticle(article: str):
    ...
    steps.append(20%threads_count)
    for step in steps:
        results.extend(multithreadExec([[requests.get, [f"https://basket-{('0' + str(i)) if (i < 10) else str(i)}.wbbasket.ru/vol{str(int(full_article[ : 4]))}/part{str(int(full_article[ : 6]))}/{article}/images/big/1.webp"]] for i in range(1 + len(results), 1 + len(results) + step)]))
    for result in results:
        if (result.status_code == 200):
            return result.url
    return None
...
def getFormatedProductsByQuery(query: str, latitude: str, longitude: str, max_count: int):
    ...
    for step in steps:
        products.extend(multithreadExec([[getProductData, [str(products_data[j]["id"]), latitude, longitude]] for j in range(len(products), len(products) + step)]))
    return products
